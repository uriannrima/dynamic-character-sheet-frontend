  ...
  methods: {
    ...
    removeFromCharacter: function(arrayName, removed) {
      this.character[arrayName] = _.filter(this.character[arrayName], p => {
        return (
          p._id !== removed._id ||
          (removed.subValue && removed.subValue !== p.subValue)
        );
      });
    },
    ...
    addSpell: function(spellAdded) {
      var spellList = _.filter(
        this.character.spellLists,
        o => o.level === spellAdded.level
      )[0];
    ...
    removeSpell: function(spellRemoved) {
      var spellList = _.filter(
        this.character.spellLists,
        o => o.level === spellRemoved.level
      )[0];
      var spellIndex = _.findIndex(
        spellList.spells,
        spell => spell._id === spellRemoved._id
      );
