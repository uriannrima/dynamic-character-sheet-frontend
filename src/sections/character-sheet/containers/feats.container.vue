    ...
    <div>
        <div class="feats-container">
            <div class="feats-header black-box">
                ...
                <span class="add-feat-icon glyphicon glyphicon-plus" v-if="addEnabled" @click="showModal = true"></span>
            </div>
        ...
        <dcs-feat-modal v-if="addEnabled" :show.sync="showModal" :describe.sync="selected" :reference-list="character.feats" @onAdded="$emit('onFeatAdded', $event)" @onUpdated="$emit('onFeatUpdated', $event)" @onRemoved="$emit('onFeatRemoved', $event)"></dcs-feat-modal>
    </div>
</template>